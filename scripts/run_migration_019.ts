...
async function main() {
  ...
  try {
      ...
      if (ipAddresses && ipAddresses.length > 0) {
          ...
          // Transaction pooler (6543) often gives "Tenant or user not found" with some clients/modes
          if (url.port === '6543') {
  ...
  try {
    const migrationPath = path.join(process.cwd(), 'db', 'migrations', '019_add_payment_tracking.sql');
